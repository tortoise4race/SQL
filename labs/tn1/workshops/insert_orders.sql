...
<<NEXT_CUST>> 
  ...
  SELECT customer_id INTO v_cust_id
     FROM (SELECT customer_id FROM CUSTOMERS SAMPLE(1) ORDER BY DBMS_RANDOM.VALUE)
     WHERE rownum <2;
  EXCEPTION
  ...
  new_order_total := 0;
   ...
   FOR i in 1 .. number_of_items LOOP
	...
	FROM (SELECT PRODUCT_ID, LIST_PRICE FROM PRODUCT_INFORMATION SAMPLE(1)
...
--     DBMS_OUTPUT.PUT_LINE('order_id='||new_order_id);
  INSERT INTO ORDERS (ORDER_ID, ORDER_DATE, ORDER_MODE, CUSTOMER_ID, 
			ORDER_STATUS, ORDER_TOTAL, SALES_REP_ID) 
	VALUES (new_order_id , SYSTIMESTAMP, 'online', v_cust_id, 1,
		new_order_total, new_sales_rep);
  COMMIT;

  FOR order_item_rec in (SELECT product_id, quantity from order_items
                           WHERE order_id = new_order_id) LOOP
